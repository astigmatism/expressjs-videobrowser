				...
				switch (keyCode) {
					...
					case 40:
						if (t.getDuration() !== Infinity) {
							seekTime -= (seekBackward * playbackRate);
						}
						break;
					case 39:
					case 38:
						if (t.getDuration() !== Infinity) {
							seekTime += (seekForward * playbackRate); //move forward slower when slowed down
						}
						break;
					case 36:
						seekTime = 0;
	...
	keyActions: [{
		...
		action: function action(player, media) {
			if (!isNaN(media.duration) && media.duration > 0) {
                ...
                if (media.paused) {
                    var newTime = media.currentTime -= (1 / 29.97);
                } else {
                    var newTime = Math.max(media.currentTime - (player.options.defaultSeekBackwardInterval(media) * media.playbackRate), 0);
                }
		...
		action: function action(player, media) {
			...
			if (!isNaN(media.duration) && media.duration > 0) {
                ...
                if (media.paused) {
                    ...
                    var newTime = Math.min(media.currentTime + (player.options.defaultSeekForwardInterval(media) * media.playbackRate), media.duration); //adding playbackRate
                }
				media.setCurrentTime(newTime);
			}
		}
